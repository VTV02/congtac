...
function validateFormData(data) {
  ...
  if (!data.phongBan) {
    return { valid: false, error: 'Vui lòng chọn phòng ban!' };
  }
  if (!data.ngayDen) {
...
function kiemTraDuLieuDangKy() {
  ...
  if (data.length <= 1) {
    Logger.log('⚠️ Sheet chỉ có header, CHƯA CÓ ĐĂNG KÝ NÀO!');
...
function kiemTraQuyenCuaToi() {
  ...
  if (result.success) {
    ...
    if (result.total > 0) {
      ...
      Logger.log('⚠️ Không tìm thấy đăng ký nào bạn có quyền xem!');
    }
  } else {
